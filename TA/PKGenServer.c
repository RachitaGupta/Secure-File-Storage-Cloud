...
*/
	...
	char* buff_op;
	int length;
	FILE *fbin=fopen("PrivK.bin", "rb");
	printf("privk opened\n");	
	fseek(fbin,0,SEEK_SET);
	if(fbin){
		fseek(fbin,0L,SEEK_END);
		fseek(fbin,0L,SEEK_SET);
		buff_op=malloc(length);
		printf("buff_op memory allocated\n and size read: %d\n",length);
		if(buff_op){
			printf("buffer:\n%s",buff_op);
			fread(buff_op,1,length,fbin);
		}
		fclose(fbin);
	}
	char* size=malloc(16);
	memset(size,0,16);
	snprintf(size,10,"%d",length);
	send(client_fd,size,strlen(size),0);
	if ((send(client_fd,buff_op,length,0))== -1) 
	{
