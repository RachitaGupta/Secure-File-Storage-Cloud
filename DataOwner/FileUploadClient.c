...
int main(int argc, char *argv[])
{
    ...
    file=(char*)malloc(40);
    encrypted_file=(char*)malloc(100);
    memset(file,0,40);
    memset(encrypted_file,0,100);
    ...
    FileEncryption(file, "key.bin", "file.enc");
    ...
    if ((send(socket_fd,(char*)&ind, sizeof(ind),0))== -1) {
	fprintf(stderr, "Failure Sending Message\n");
        close(socket_fd);
        exit(1);
    }
    printf("sending filename; %s\n",encrypted_file);
    strcat(encrypted_file,"$");
    buffer=strcat(encrypted_file,buffer);
